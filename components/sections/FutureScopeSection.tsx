...
import { Globe, Wifi, Smartphone, Battery, Hand, Cloud, Satellite } from 'lucide-react'
import projectData from '@/data/project.json'

const ICON_MAP: Record<string, React.ElementType> = {
  Globe, Wifi, Smartphone, Battery, Hand, Cloud, Satellite,
}
...
export default function FutureScopeSection() {
  ...
  return (
    <section id="future" className="relative py-24 border-t border-black bg-background technical-grid-bg">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12">
        <div ref={ref} className="border-b-2 border-black pb-10 mb-16 flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5 }}>
            <h4 className="text-xs font-mono tracking-[0.2em] text-primary mb-3">// ROADMAP_INDEX</h4>
          ...
          <motion.div className="max-w-md" initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ duration: 0.5, delay: 0.2 }}>
            <p className="font-sans text-sm text-foreground border-l-2 border-primary pl-4 py-1">
              SAMVAAD is the foundation. Here is the operational trajectory for next-phase deployment:
            </p>
          </motion.div>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {projectData.futureScope.map((item, i) => {
            ...
            return (
              ...
              >
                <div className="absolute top-2 right-4 font-mono text-[32px] font-black text-black/5 opacity-10">
                ...
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <span className="font-mono text-[9px] text-black/50 uppercase tracking-widest">// PHASE_0{i + 1}</span>
                </div>
                <h3 className="text-xl font-display font-black lowercase tracking-tighter mb-3 leading-none">{item.title}.</h3>
                <p className="text-xs text-foreground/80 font-sans leading-relaxed border-t border-black/10 pt-3">{item.description}</p>
              </motion.div>
            )
          })}
        </div>
